...
用户设置/期望 max_turns=500（`AgentConfig` 默认，config.py:100），但 `vague-code <task>` 与 `vague-code tui` 入口的 argparse `--max-turns` 默认 20（vague_code/cli/__init__.py:33、:141）覆盖了配置默认，导致未传参时 20 轮即熔断。
...
- `vague_code/cli/__init__.py` 两处 `--max-turns` argparse `default=20` → `default=None`
...
- `vague-code "task"`（不传参）构造的 config.max_turns == 500
- `vague-code "task" --max-turns 30` 构造的 config.max_turns == 30
- ruff check
