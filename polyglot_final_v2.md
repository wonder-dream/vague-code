...
总任务数: 225
总运行次数: 225
总成本: $9.6936（按评测时 cli 单价估）

## 汇总

| 压缩 | 并发 | RepoMap | 重复 | 通过率 | 平均轮次 | 平均 input tokens | code_search | stale回收 | micro回收 | ssnip回收 | auto回收 | truncate回收 | 成本($) |
|------|------|---------|------|--------|----------|-------------------|-------------|-----------|-----------|-----------|----------|--------------|---------|
| ✓ | ✓ | ✓ | 0 | 96% | 10.4 | 119,563 | 0 | 0 | 0 | 0 | 0 | 0 | $9.69 |
## 指标口径（ADR-0040）

| 指标 | 数值 | 分母 | 含义 |
|------|------|------|------|
| pass@1 | 96.00% | 225 题（有明确判分） | 模型代码能力口径 |
| e2e mean | 96.00% | 225 题（全题） | 整条链路成功率（异常按 0） |

## 失败分类分账（互斥分类学）

| 类别 | 数量 | 占比 | 归因 |
|------|------|------|------|
| 成功（verified） | 216 | 96% | 模型能力（通过） |
| 改错（有产出，F2P 挂） | 6 | 3% | 模型能力 |
| 超时（预算耗尽） | 3 | 1% | 预算耗尽（超时） |
...
| 指标 | p50 | p90 | max |
|------|-----|-----|-----|
| input tokens | 72,277 | 221,770 | 1,142,509 |
| output tokens | 6,225 | 16,621 | 65,713 |
| cache-hit tokens | 64,896 | 213,120 | 1,128,704 |
| cost (USD) | 0.0271 | 0.0850 | 0.3834 |
...
|--------|------|------|----------|------|------|--------------|----------------|
| cpp/all-your-base | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 10 | 52,337 | end_turn |
| cpp/allergies | compression=1_concurrency=1_repo_map=1 | ✗ | ✗ | verify:fail(exit 1) | 18 | 484,559 | end_turn |
| cpp/bank-account | compression=1_concurrency=1_repo_map=1 | ✗ | ✗ | verify:fail(exit 1) | 9 | 61,876 | end_turn |
| cpp/binary-search-tree | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 13 | 133,041 | end_turn |
| cpp/circular-buffer | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 10 | 62,286 | end_turn |
| cpp/clock | compression=1_concurrency=1_repo_map=1 | ✗ | ✗ | verify:fail(exit 1) | 10 | 50,681 | end_turn |
| cpp/complex-numbers | compression=1_concurrency=1_repo_map=1 | ✗ | ✗ | verify:fail(exit 2) | 13 | 103,929 | end_turn |
| cpp/crypto-square | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 9 | 75,449 | end_turn |
| cpp/diamond | compression=1_concurrency=1_repo_map=1 | ✗ | ✗ | verify:fail(exit 1) | 9 | 48,235 | end_turn |
| cpp/dnd-character | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 7 | 35,086 | end_turn |
| cpp/gigasecond | compression=1_concurrency=1_repo_map=1 | ✗ | ✗ | verify:fail(exit 1) | 40 | 804,884 | max_turns |
| cpp/grade-school | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 11 | 76,378 | end_turn |
| cpp/kindergarten-garden | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 9 | 56,125 | end_turn |
| cpp/knapsack | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 7 | 27,587 | end_turn |
| cpp/linked-list | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 9 | 59,024 | end_turn |
| cpp/meetup | compression=1_concurrency=1_repo_map=1 | ✗ | ✗ | verify:fail(exit 1) | 40 | 851,178 | max_turns |
| cpp/parallel-letter-frequency | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 10 | 106,497 | end_turn |
| cpp/perfect-numbers | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 10 | 50,795 | end_turn |
| cpp/phone-number | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 8 | 52,958 | end_turn |
| cpp/queen-attack | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 67,593 | end_turn |
| cpp/robot-name | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 11 | 92,684 | end_turn |
| cpp/space-age | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 10 | 63,954 | end_turn |
| cpp/spiral-matrix | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 7 | 28,414 | end_turn |
| cpp/sublist | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 11 | 86,386 | end_turn |
| cpp/yacht | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 8 | 50,246 | end_turn |
| cpp/zebra-puzzle | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 10 | 84,633 | end_turn |
...
| javascript/affine-cipher | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 14 | 113,555 | end_turn |
| javascript/alphametics | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 13 | 192,707 | end_turn |
| javascript/beer-song | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 14 | 163,231 | end_turn |
| javascript/binary | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 13 | 53,940 | end_turn |
| javascript/book-store | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 14 | 142,733 | end_turn |
| javascript/bottle-song | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 82,088 | end_turn |
| javascript/bowling | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 15 | 391,283 | end_turn |
| javascript/complex-numbers | compression=1_concurrency=1_repo_map=1 | ✗ | ✗ | verify:fail(exit 1) | 40 | 792,810 | max_turns |
| javascript/connect | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 176,257 | end_turn |
| javascript/food-chain | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 15 | 136,575 | end_turn |
| javascript/forth | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 140,153 | end_turn |
| javascript/go-counting | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 118,630 | end_turn |
| javascript/grade-school | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 15 | 275,754 | end_turn |
| javascript/grep | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 17 | 221,770 | end_turn |
| javascript/house | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 11 | 73,105 | end_turn |
| javascript/killer-sudoku-helper | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 15 | 94,468 | end_turn |
| javascript/ledger | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 21 | 227,965 | end_turn |
| javascript/list-ops | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 11 | 68,636 | end_turn |
| javascript/meetup | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 109,103 | end_turn |
| javascript/ocr-numbers | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 13 | 88,646 | end_turn |
| javascript/palindrome-products | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 18 | 267,591 | end_turn |
| javascript/parallel-letter-frequency | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 11 | 108,708 | end_turn |
| javascript/phone-number | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 65,759 | end_turn |
| javascript/pig-latin | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 15 | 126,258 | end_turn |
| javascript/poker | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 148,414 | end_turn |
| javascript/promises | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 13 | 97,957 | end_turn |
| javascript/queen-attack | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 11 | 74,978 | end_turn |
| javascript/rational-numbers | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 22 | 288,776 | end_turn |
| javascript/react | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 18 | 303,145 | end_turn |
| javascript/rectangles | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 82,642 | end_turn |
| javascript/resistor-color-trio | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 13 | 103,054 | end_turn |
| javascript/rest-api | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 14 | 138,394 | end_turn |
| javascript/robot-name | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 61,039 | end_turn |
| javascript/say | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 82,110 | end_turn |
| javascript/scale-generator | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 16 | 196,902 | end_turn |
| javascript/simple-linked-list | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 11 | 58,099 | end_turn |
| javascript/space-age | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 14 | 72,277 | end_turn |
| javascript/state-of-tic-tac-toe | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 161,715 | end_turn |
| javascript/sum-of-multiples | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 11 | 70,467 | end_turn |
| javascript/tournament | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 13 | 113,904 | end_turn |
| javascript/transpose | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 14 | 177,038 | end_turn |
| javascript/triangle | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 11 | 76,977 | end_turn |
| javascript/twelve-days | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 20 | 189,405 | end_turn |
| javascript/two-bucket | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 13 | 168,125 | end_turn |
| javascript/variable-length-quantity | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 12 | 93,313 | end_turn |
| javascript/word-search | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 11 | 96,711 | end_turn |
| javascript/wordy | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 14 | 122,466 | end_turn |
| javascript/zebra-puzzle | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 18 | 214,118 | end_turn |
| javascript/zipper | compression=1_concurrency=1_repo_map=1 | ✓ | ✓ | verify:pass | 11 | 68,345 | end_turn |
...
| rust/pig-latin | compression=1_concurrency=1_repo_map=1 | ✗ | ✗ | verify:fail(exit 101) | 3 | 7,494 | max_tokens |
...
|------|---|------------|----------|--------|-------------------------------|
| compression=1_concurrency=1_repo_map=1 | 1 | 216 | 225 | 96% | 96%（216/225） |

整体 pass^k: 216/225 = 96%
