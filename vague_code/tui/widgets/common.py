"""Textual widgets used by the vague-code TUI.

Ported from the firstcoder TUI reference (app/tui_widgets.py) with the
FirstCoder* classes renamed to vague-code*.
"""
...
class VagueCodeMarkdown(Markdown):
...
VagueCodeMarkdown.BLOCKS = {
    name: type(
        f"VagueCode{block.__name__}",
        (block,),
        {"ALLOW_SELECT": True},
    )
...
class VagueCodeScreen(Screen[None]):
    ...
    def _selection_is_blocked_by_streaming_markdown(widget) -> bool:
        ...
        while parent is not None:
            if isinstance(parent, VagueCodeMarkdown) and not parent.allow_select:
                return True
