...
总任务数: 225
总运行次数: 225
总成本: $9.6681（按评测时 cli 单价估）

## 汇总

| 压缩 | 并发 | RepoMap | 重复 | 通过率 | 平均轮次 | 平均 input tokens | code_search | stale回收 | micro回收 | ssnip回收 | auto回收 | truncate回收 | 成本($) |
|------|------|---------|------|--------|----------|-------------------|-------------|-----------|-----------|-----------|----------|--------------|---------|
| ✓ | ✓ | ✓ | 0 | 99% | 10.4 | 118,751 | 0 | 0 | 0 | 0 | 0 | 0 | $9.67 |
## 指标口径（ADR-0040）

| 指标 | 数值 | 分母 | 含义 |
|------|------|------|------|
| pass@1 | 99.11% | 224 题（有明确判分） | 模型代码能力口径 |
| e2e mean | 98.67% | 225 题（全题） | 整条链路成功率（异常按 0） |

## 失败分类分账（互斥分类学）

| 类别 | 数量 | 占比 | 归因 |
|------|------|------|------|
| 成功（verified） | 222 | 99% | 模型能力（通过） |
| 改错（有产出，F2P 挂） | 1 | 0% | 模型能力 |
| 超时（预算耗尽） | 1 | 0% | 预算耗尽（超时） |
...
| 指标 | p50 | p90 | max |
|------|-----|-----|-----|
| input tokens | 74,926 | 221,770 | 1,142,509 |
| output tokens | 6,168 | 16,621 | 65,713 |
| cache-hit tokens | 69,248 | 213,120 | 1,128,704 |
| cost (USD) | 0.0282 | 0.0797 | 0.3834 |
...
|------|---|------------|----------|--------|-------------------------------|
| compression=1_concurrency=1_repo_map=1 | 1 | 222 | 224 | 99% | 99%（222/224） |

整体 pass^k: 222/224 = 99%
