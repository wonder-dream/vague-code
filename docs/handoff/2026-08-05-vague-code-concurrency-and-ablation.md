...
| 修复 P1 | `_normalize_path` 在 `os.name=="nt"` 时归一化为小写：`SRC/A.PY` vs `vague_code/a.py` 在 Windows 上同一文件，此前判为不冲突；glob prefix 同样归一化（`_pattern_prefix` 只换斜杠） | concurrency.py:50-55, 82 |
...
# 静态检查
uv run ruff check vague_code/agent/concurrency.py
uv run mypy vague_code/agent/concurrency.py   # 注意：loop.py:315 有既有 mypy error（guidance str/list），与本次无关
