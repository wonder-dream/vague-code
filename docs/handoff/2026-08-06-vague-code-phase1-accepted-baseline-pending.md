...
1. **后台长跑的正确姿势**：CLI 串行 for 循环（harness.py:364），并行=多进程；WMI 启动可脱离 bash 工具进程树（`([wmiclass]'Win32_Process').Create('cmd /c cd /d D:\document\vague-code && python -m eval.cli ... > log 2>&1')`）；`Start-Process`/`cmd start /b` 启动的进程会被 bash 工具超时连带杀掉
...
- 上一交接：`docs/handoff/2026-08-05-vague-code-supervision-implementation.md`（P1 口径、坑、命令）
