...
**实现文件清单**：`src/agent/config.py`（SupervisionConfig）→ `src/agent/ir.py`（EventType.supervision）→ `src/agent/loop.py`（三处钩子：周期监督/完成校验/stuck 累计）→ `src/agent/tools.py`（bash 测试解析）→ `eval/harness.py`（--supervisor 开关 + supervision 事件统计）→ `eval/classify.py`（stagnant/supervisor_done 分类）→ `eval/metrics.py`（supervision 事件处理）。
