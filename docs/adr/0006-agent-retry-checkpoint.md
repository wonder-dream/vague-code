...
当前 Loop �?catch 异常后写 error + run_end 就退出，不做重试。同时进程崩溃后无法从已有轨迹恢复继续执行�?

�?ADR 覆盖 Week 1 路线�?指数退�?+ 抖动重试"�?检查点与回退"两项�?

## Considered Options

| 决策�?| 选项 | 选出方案 |
|--------|------|----------|
| 重试所在层 | A: �?Backend �?/ B: 两层分工（SDK + Loop�?| B |
| 流中断重试策�?| A: 续传 / B: 从头重试 | B（LLM 非确定性） |
| 响应分歧处理 | A: 接受新结�?/ B: 检测并记录 | B |
| 检查点时刻 | A: 工具执行�?/ B: 工具执行前（事务语义�?| B |
| 恢复�?config 变化 | A: 拒绝 / B: 允许+告警 | B |
| retry 消融控制 | A: 硬编�?/ B: AgentConfig 暴露 | B |
...
| �?| 职责 | 机制 | �?Loop |
|----|------|------|---------|
| SDK �?| 瞬时网络抖动 | `OpenAI(max_retries=2)` �?SDK 自动重试 429/500/502/503/连接错误 | 完全透明 |
| Loop �?| 策略级重�?| 指数退�?+ 全抖动，retry budget 可控 | emit `retry` 事件入轨�?|

**为什么两层不是一�?*：SDK 层重试对 Loop 完全透明，基础设施噪声被吸收。Loop 层重试做策略决策——是否重试可�?AgentConfig 开/关（消融实验变量）、有上下文（当前 turn、剩�?budget）、厂商无关、可审计记录�?
...
评测消融实验�?`retry_enabled=False` 可对比重试对通过率的贡献�?

### 4. 重试行为规则

| 规则 | 说明 |
|------|------|
| 不消�?turn | 重试在同一轮内循环，turn 不递增 |
| 流中断从头重�?| 废弃已缓冲区 delta，重新调�?`backend.stream()`——LLM 是概率模型，每次回答可能不同 |
| 不重�?budget | 同一轮内重试共享同一�?`max_turns` 计数�?token 累计 |
| retry budget 耗尽 | 全部重试耗尽 �?emit `error(retry_exhausted, attempts=5)` �?`run_end(llm_error)` |

### 5. 响应分歧检�?

重试同一�?messages 列表，模型可能返回不同的结果（非确定性）�?

- 比较两次响应�?`stop_reason` �?`tool_use` 列表是否相同
- 不一致时 emit `retry_divergence` 事件，payload 含前后差异摘�?
- 不影响主流程——接受新响应继续执行

### 6. 重试事件

每次重试 emit 一�?`retry` 事件�?
...
2. messages.append(resp.message)                �?assistant 消息已就�?
3. traj.persist()                                �?检查点
...
**恢复语义**：若进程�?4/5/6 崩溃，从检查点恢复�?assistant 消息中的 tool_use 都未执行过（步骤 3 时尚未执行）。这�?tool_use 全部重做——事务要么全成功，要么全回滚�?
...
不重放历�?LLM 调用——直接从 `to_messages()` 重建 messages，继续下一轮�?

### 9. 恢复边界处理

| 场景 | 处理 |
|------|------|
| config.model 不一�?| �?允许，emit warning |
| config.tools 不一�?| �?`ValueError`——tools 列表变化可能导致未知工具 |
| workdir 文件变化 | �?文件不存�?�?handler 返回 is_error �?模型自愈 |
| 跨会话污�?| �?不可能——恢复按 `run_id` 隔离 |
| 已有 run_end | �?status �?`end_turn/max_turns/llm_error` 等——不可恢�?|
| �?run_end（进程崩溃） | �?status �?`in_progress`——可恢复 |
...
- 重试机制可控可消融——评测可关重试验�?重试对通过率的贡献"
- 每次重试独立入轨迹——可精确回答"多少任务触发过重试、平均延迟多少秒"
- 检查点使长任务可恢复—�?0+ 轮会话不会因进程崩溃全部丢失
- 恢复引入 `Trajectory.from_db()` �?`Agent.resume()`，保持向后兼容（功能不修现有接口�?
- SDK 层和 Loop 层重试分工明确，异常分类细化�?10 个细�?+ 1 兜底

## 修正案（2026-07-23�?

### 修正�?1：配置归属以 ADR-0005 为准

§3 的配置字段归属以 ADR-0005 §16 为准——retry/timeout 旋钮�?`TransportConfig`
...
原文 §3 表格�?`AgentConfig` 字段表述作废�?

### 修正�?2：retry budget 耗尽后保留精确终�?

§4 retry budget 耗尽的终态修正——保留精确终态：
- last error �?`APITimeoutError` �?`run_end(llm_timeout)`
- 其余可重试错误耗尽 `run_end(llm_error)`

`error(kind=retry_exhausted)` �?payload 携带 `last_error_kind`�?
理由：轨迹是评测唯一数据源，失败分类信息不丢失�?

### 修正�?3：工具执�?at-least-once 语义（已知限制）

检查点设在工具执行之前（�?），因此若进程在工具执行期间崩溃，该次崩溃前已执行的工具会因回滚到检查点而被重做�?
这对有副作用（写文件、bash 命令）的工具意味着可能执行两次，属�?at-least-once 语义�?
v0 不接受此限制的工程化缓解；这�?Temporal、Airflow 等生产系统的常见议题，面试时可展开讨论�?
