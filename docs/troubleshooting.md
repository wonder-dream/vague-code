...
| `APITimeoutError` | 网络问题 | `--timeout-s 300` 或检查网络 |
...
| 症状 | 诊断 | 解决方案 |
|------|------|---------|
| `PermissionError: Path traversal detected` | Agent 尝试访问外部文件 | 检查任务描述 → 确保路径参数是相对路径 |
| `FileNotFoundError` | 文件不存在 | Agent 自动重新检查 → 正常行为 |
| 多次匹配错误（patch） | old_str 不够精确 | Agent 会自动增加上下文 → 正常行为 |
| `RuntimeError: command timed out` | 命令超 30 秒 | Agent 会尝试更简单的命令 |
...
这是文档 `troubleshooting.md` 的细纲。实际写作时需每一步在目标环境下手动验证解决方案。
