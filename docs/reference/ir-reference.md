...
| MessageEnd | `MessageEnd(stop_reason, finish_reason, truncated, usage)` | `{"stream_type": "message_end", "stop_reason": "...", "usage": {...}, "truncated": false}` | 9 |
...
**`dispatch_event()` 分发（`ir.py:277-297`）：** `isinstance` 链式分发到 `StreamEventVisitor` 协议方法

**`StreamEventVisitor` Protocol（`ir.py:264-274`）：** 10 个方法，CLI/TUI 渲染器实现

**`StreamDisconnect`：** 流异常断开时抛出（`ir.py:258-259`）

---

## 结尾

**下一篇推荐：** → R3：Tool API 参考

---

## 本文件说明

这是文档 `ir-reference.md` 的细纲。实际写作时需与 `ir.py` 逐行核对所有 dataclass 字段。
