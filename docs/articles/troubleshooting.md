...
| 症状 | 诊断 | 解决方案 |
|------|------|---------|
| system prompt 无「项目记忆」段 | `.agent/memory.md` 不存在或为空 | 需要会话结束蒸馏（或长会话触发 auto_compact）后才产生记忆 |
| 记忆过时/错误 | 文件式记忆不做自动冲突检测 | 直接编辑 `.agent/memory.md` 增删改（文件即事实源） |
| 记忆文件超过 200 行/25KB | 注入截尾，尾部内容不可见 | 手动精简 `.agent/memory.md` 或删除过时条目 |
